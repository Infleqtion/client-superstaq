        ...
        if index is None:
            if all(self._job[job_id].get("pulse_gate_circuits") for job_id in job_ids):
                ...
                for serialized_circuit in serialized_circuits:
                    deserialized_circuit = gss.serialization.deserialize_qiskit_circuits(
            ...
            return gss.serialization.deserialize_qiskit_circuits(
                serialized_circuit, circuits_is_list=False
            )
...
class JobV3(gss.job.Job):
    ...
    def num_qubits(self, index: int | None = None) -> int | list[int]:
        ...
        """
        if index is None:
            return [self.num_qubits(i) for i in range(self.job_data.num_circuits)]

        return cirq.num_qubits(self.input_circuits(index))
    ...
    def compiled_circuits(
        self, index: int | None = None, *, timeout_seconds: int = 7200, polling_seconds: float = 1.0
    ) -> cirq.Circuit | list[cirq.Circuit]:
        """Gets the compiled circuits that were processed for this job.
        ...
        if (serialized_circuit := self.job_data.compiled_circuits[index]) is None:
            raise gss.SuperstaqException(
    ...
    def counts(
        ...
        if index is None:
            return [
                self.counts(i, timeout_seconds, polling_seconds, qubit_indices)
