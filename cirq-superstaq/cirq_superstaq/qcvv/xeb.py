...
class XEB(BenchmarkingExperiment):
    ...
    @property
    def results(self) -> XEBResults:
        """The results from the most recently run experiment"""
        return cast("XEBResults", super().results)
    ...
    def build_circuits(
        ...
        Args:
            ...
            layers: An iterable of the different numbers of layers to include in each circuit.

        Returns:
            A list of randomly generated circuit samples.
        """
        random_circuits = []
        for _, depth in product(range(num_circuits), layers, desc="Building circuits"):
            ...
            if self.two_qubit_gate is not None:
                circuit = self._interleave_gate(circuit, self.two_qubit_gate)
    ...
    def process_probabilities(self) -> None:
        ...
        self.sample_circuits_with_simulator(
            target=cirq.Simulator(),
            shots=10_000,
        )

        for sample in self.samples:
            sample.target_probabilities = sample.probabilities
            sample.probabilities = {}
    ...
    def analyse_results(self, plot_results: bool = True) -> XEBResults:
        ...
        Returns:
            XEBResults: The final results from the experiment.
        """
