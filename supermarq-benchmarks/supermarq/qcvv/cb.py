...
class CBResults(QCVVResults):
    ...
    def undressed_process_fidelity(self) -> float:
        ...
        Raises:
            ...
            RuntimeError: If the experiment only concerns the dressed process.
        """
    ...
    def _compute_mean_expectations(self) -> pd.DataFrame:
        ...
        if self.experiment._undressed_process:
            ...
            for d in depths:
                ...
                mean_values["std_exp", d, "undressed"] = mean_values[
                    "mean_exp", d, "undressed"
                ] * np.sqrt(
                    (mean_values["std_exp", d, "identity"] / mean_values["mean_exp", d, "identity"])
                    ** 2
    ...
    def _compute_fidelities(self, expectations: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        """
        depths = expectations.columns.get_level_values("cycle_depth").unique()
        fid = (
            (
                (expectations.mean_exp[depths[0]] / expectations.mean_exp[depths[1]])
                ** (1 / (depths[0] - depths[1]))
            )
            .reset_index()
...
class CB(QCVVExperiment[CBResults]):
    ...
    This experiment implements the cycle benchmarking protocol to estimate the fidelity of a
    Clifford process.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialises a Cycle Benchmarking experiment.
    ...
    @property
    def sorted_qubits(self) -> list[cirq.Qid]:
        """Returns the qubits sorted in ascending order."""
    ...
    def _apply_circuit_to_tableau(circuit: cirq.Circuit, tableau: cirq.CliffordTableau) -> None:
        ...
        for op in circuit.all_operations():
            if isinstance(op.gate, cirq.PhasedXZGate):
                ...
                raise TypeError("gate must be either `PhaseXZGate` or `CZPowGate`.")
