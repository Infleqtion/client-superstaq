...
class XEB(BenchmarkingExperiment[XEBResults]):
    ...
    @staticmethod
    def simulate_sample(sample) -> dict[str, float]:
    ...
    def process_probabilities(self, samples: Sequence[XEBSample]) -> pd.DataFrame:
        ...
        for sample in tqdm(samples, desc="Evaluating circuits"):
            sample.target_probabilities = self.simulate_circuit(sample)
