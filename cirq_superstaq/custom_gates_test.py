...
def test_barrier() -> None:
    n = 3
    gate = cirq_superstaq.custom_gates.Barrier(n)
...
def test_custom_resolvers() -> None:
    circuit = cirq.Circuit()
    qubits = cirq.LineQubit.range(2)
    circuit += cirq_superstaq.custom_gates.FermionicSWAPGate(1.23)(*qubits)
    circuit += cirq_superstaq.custom_gates.Barrier(2)(*qubits)
    circuit += cirq.CX(*qubits)

    json_text = cirq.to_json(circuit)
    resolvers = [cirq_superstaq.custom_gates.custom_resolvers, *cirq.DEFAULT_RESOLVERS]
