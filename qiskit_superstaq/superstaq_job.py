...
class SuperstaQJob(qiskit.providers.JobV1):
    ...
    def _wait_for_results(self, timeout: float = None, wait: float = 5) -> List[Dict]:
        ...
        for jid in job_ids:
            ...
            while True:
                ...
                getstr = f"{self._backend.url}/" + qss.API_VERSION + f"/jobs/{jid}"
                result = requests.get(
                    getstr, headers=header, verify=bool(self._backend.url == qss.API_URL)
                ).json()
    ...
    def status(self) -> str:
        ...
        for job_id in job_id_list:
            get_url = qss.API_URL + "/" + qss.API_VERSION + f"/jobs/{job_id}"
            result = requests.get(
                get_url,
                headers=header,
            )
