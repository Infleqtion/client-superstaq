...
def test_service_run() -> None:
    service = Service(remote_host="http://example.com", api_key="key")
...
@mock.patch(
    "superstaq_client._SuperstaQClient.find_min_vol_portfolio",
...
def test_service_find_min_vol_portfolio(mock_find_min_vol_portfolio: mock.MagicMock) -> None:
    service = Service(remote_host="http://example.com", api_key="key")
    expected = finance.MinVolOutput(["AAPL", "GOOG"], 8.1, 10.5)
    assert service.find_min_vol_portfolio(["AAPL", "GOOG", "IEF", "MMM"], 8) == expected


@mock.patch(
    "superstaq_client._SuperstaQClient.find_max_pseudo_sharpe_ratio",
    return_value={
...
def test_service_find_max_pseudo_sharpe_ratio(
    ...
    service = Service(remote_host="http://example.com", api_key="key")
    expected = finance.MaxSharpeOutput(["AAPL", "GOOG"], 8.1, 10.5, 0.771)
    assert service.find_max_pseudo_sharpe_ratio(["AAPL", "GOOG", "IEF", "MMM"], k=0.5) == expected


@mock.patch(
    "superstaq_client._SuperstaQClient.tsp",
    return_value={
...
@mock.patch(
    "superstaq_client._SuperstaQClient.warehouse",
    return_value={
...
def test_service_warehouse(mock_warehouse: mock.MagicMock) -> None:
    service = Service(remote_host="http://example.com", api_key="key")
    expected = logistics.WarehouseOutput(
...
def test_service_api_key_via_env() -> None:
    ...
    service = Service(remote_host="http://example.com")
...
def test_service_remote_host_via_env() -> None:
    ...
    service = Service(api_key="tomyheart")
...
def test_service_no_param_or_env_variable() -> None:
    with pytest.raises(EnvironmentError):
        _ = Service(remote_host="http://example.com")
