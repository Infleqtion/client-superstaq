...
def test_superstaq_client_create_job_invalid_json(
    mock_post: mock.MagicMock, api_version: str
) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.get")
def test_superstaq_client_get_balance(mock_get: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
    ...
    if api_version == "v0.2.0":
        ...
        response = client.get_balance()
        mock_get.assert_called_with(
            f"http://example.com/{api_version}/client/balance",
            headers=EXPECTED_HEADERS[api_version],
            verify=False,
        )
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.get")
def test_superstaq_client_get_version(mock_get: mock.MagicMock, api_version: str) -> None:
    mock_get.return_value.ok = True
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_add_new_user(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
    ...
    if api_version == "v0.2.0":
        ...
        user_data = {"name": "Marie Curie", "email": "mc@email.com", "role": "genius"}
        client.add_new_user(user_data)
        mock_post.assert_called_with(
            f"http://example.com/{api_version}/client/user",
            headers=EXPECTED_HEADERS[api_version],
            json=user_data,
            verify=False,
        )
...
def test_update_user_balance(
    mock_post: mock.MagicMock, mock_put: mock.MagicMock, api_version: str
) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
    ...
    if api_version == "v0.2.0":
        ...
        mock_put.assert_called_with(
            f"http://example.com/{api_version}/client/user/mc@gmail.com",
            headers=EXPECTED_HEADERS[api_version],
            json=expected_json,
            verify=False,
        )
...
def test_update_user_balance_invalid_v3() -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
    ...
    with pytest.raises(ValueError, match="user email"):
        client.update_user_balance({"balance": 5.00})

    with pytest.raises(ValueError, match="new balance"):
        client.update_user_balance({"email": "test@email.com"})
...
def test_update_user_role(
    ...
    if api_version == "v0.2.0":
        ...
        client.update_user_role({"email": "mc@gmail.com", "role": "genius"})
        expected_json = {"role": "genius"}
        mock_put.assert_called_with(
            f"http://example.com/{api_version}/client/user/mc@gmail.com",
            headers=EXPECTED_HEADERS[api_version],
            json=expected_json,
...
def test_update_user_role_invalid_v3() -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
    ...
    with pytest.raises(ValueError, match="user email"):
        client.update_user_role({"role": "genius"})

    with pytest.raises(ValueError, match="new role"):
        client.update_user_role({"email": "test@email.com"})


@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_resource_estimate(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
def test_superstaq_client_get_targets(
    ...
    if api_version == "v0.2.0":
        ...
        mock_get.assert_called_once_with(
            f"http://example.com/{api_version}/client/targets",
            headers=EXPECTED_HEADERS[api_version],
            verify=False,
        )
        ...
        response = client.get_targets(simulator=True)
        mock_get.assert_called_with(
            f"http://example.com/{api_version}/client/targets?simulator=True",
            headers=EXPECTED_HEADERS[api_version],
            verify=False,
        )
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_aqt_compile(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_submit_qubo(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_supercheq(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_aces(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_cb(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.post")
def test_superstaq_client_dfe(mock_post: mock.MagicMock, api_version: str) -> None:
    client = gss.superstaq_client._SuperstaqClient(
        client_name="general-superstaq",
        remote_host="http://example.com",
        api_key="to_my_heart",
...
@pytest.mark.parametrize("api_version", ["v0.2.0", "v0.3.0"])
@mock.patch("requests.Session.get")
def test_superstaq_client_aqt_get_configs(mock_get: mock.MagicMock, api_version: str) -> None:
    expected_json = {"pulses": "Hello", "variables": "World"}
...
def test_get_user_info_query_composite(
    ...
    if api_version == "v0.2.0":
        ...
        data = [{"Some": "Data"}]
        user_id = 42
        endpoint = route + f"?name=Alice&id={user_id}"
        ...
        mock_get.return_value.json.return_value = data
        user_id = uuid.UUID(int=0)
        endpoint = route + f"?name=Alice&user_id={user_id}"
