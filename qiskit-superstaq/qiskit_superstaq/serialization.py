...
def _prepare_gate(gate: qiskit.circuit.Instruction) -> qiskit.circuit.Instruction:
    ...
    assert isinstance(gate, qiskit.circuit.Instruction)

    custom_gate_types: tuple[type[qiskit.circuit.Instruction], ...] = tuple(
        _custom_gates_by_name.values()
    )
    if isinstance(gate, custom_gate_types) and not issubclass(
    ...
    if isinstance(gate, qiskit.circuit.ControlledGate):
        definition = (
            ...
            if gate._definition is None
            else _prepare_circuit(gate._definition)
        )
